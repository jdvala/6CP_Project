...
def encoder(x):
	# Encoder Hidden layer with sigmoid activation #1
    layer_1 = tf.nn.relu(tf.add(tf.matmul(x, weights_encoder_post_1),
                                   bias_encoder_post_1))
    # Decoder Hidden layer with sigmoid activation #2
    layer_2 = tf.nn.relu(tf.add(tf.matmul(layer_1, weights_encoder_post_2),
                                   bias_encoder_post_2))
    layer_3 = tf.nn.relu(tf.add(tf.matmul(layer_2, weights_encoder_post_3),
                                   bias_encoder_post_3))
